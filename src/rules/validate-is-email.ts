 ...
 */
export function validateIsEmail(value: any, output: OutputType = 'record'): string[] {
  const errors: string[] = [];
  ...
  if (emailParts.length !== 2) {
    const errorMessage = "The email address must contain exactly one '@' symbol.";
    if (output === 'exception') {
      throw new ValidationError(errorMessage);
    } else {
      errors.push(errorMessage);
  ...
  if (account.length > 64) {
    const errorMessage = "The part before the '@' cannot have more than 64 characters.";
    if (output === 'exception') {
      throw new ValidationError(errorMessage);
    } else {
      errors.push(errorMessage);
  ...
  if (address.length > 255) {
    const errorMessage = "The part after the '@' cannot have more than 255 characters.";
    if (output === 'exception') {
      throw new ValidationError(errorMessage);
    } else {
      errors.push(errorMessage);
  ...
  if (domainParts.some((part) => part.length > 63)) {
    const errorMessage = 'No part of the domain can have more than 63 characters.';
    if (output === 'exception') {
      throw new ValidationError(errorMessage);
    } else {
      errors.push(errorMessage);
